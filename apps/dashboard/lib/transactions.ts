...
function friendlyError(message: string): Error {
  ...
  if (message.includes("event membership")) {
    return new Error(
      "This movement belongs to an event. Treatment cannot be changed here.",
    );
  }
...
async function readRpc<T>(
  ...
  if (!READ_RPCS.has(name) && process.env.NODE_ENV === "production") {
...
export async function listTransactions(input: {
  ...
  return {
    items: Array.isArray(record.items)
      ? (record.items as TransactionFeedItem[])
      : [],
    nextCursor:
...
export async function getTransactionFilters(): Promise<{
  ...
  return {
    accounts: Array.isArray(record.accounts)
      ? (record.accounts as AccountOption[])
      : [],
    categories: Array.isArray(record.categories)
      ? (record.categories as CategoryOption[])
      : [],
  };
}
...
export function setTransactionCategory(input: {
  ...
  return writeRpc("finance_set_transaction_category_v1", {
    ...
    p_expected_confirmed_classification_id:
      input.expectedConfirmedClassificationId,
    p_expected_proposed_classification_id:
      input.expectedProposedClassificationId,
