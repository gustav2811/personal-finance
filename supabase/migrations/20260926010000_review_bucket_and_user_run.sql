-- A proposal with no mapped FinWise category is not a disagreement.
-- A human classification does not inherit the classifier run id.

create or replace function finance.review_bucket(
  confirmed_id uuid,
  proposed_id uuid,
  ...
  awaiting_classifier boolean
) returns text
language sql
immutable
as $$
  select case
    when confirmed_id is not null then 'confirmed'
    when proposed_id is not null
      and mapped_category_id is not null
      and proposed_category_id = mapped_category_id then 'jev_agrees'
    when proposed_id is not null and mapped_category_id is null then 'jev_proposed'
    when proposed_id is not null then 'jev_disagrees'
    when latest_run_status = 'failed' then 'classifier_failed'
    when latest_run_status = 'succeeded' and proposed_id is null then 'classifier_abstained'
    when mapped_category_id is null and source_category_name is null then 'unclassified'
    when awaiting_classifier then 'awaiting_classifier'
    else 'unclassified'
  end
$$;
...
as $$
  ...
  select jsonb_build_object(
    ...
    'reviewState', finance.review_bucket(
      confirmed.id,
      proposed.id,
      proposed.category_id,
      mapped.owned_category_id,
      latest_run.status,
      tx.source_category_name_snapshot,
      ...
      where t.household_id = v_household
        ...
        and (
          ...
          or (
            ...
            and finance.review_bucket(
              confirmed.id,
              proposed.id,
              proposed.category_id,
              mapped.owned_category_id,
              latest_run.status,
              t.source_category_name_snapshot,
          ...
          or (
            v_review <> 'needs_review'
            and v_review = finance.review_bucket(
              confirmed.id,
              proposed.id,
              proposed.category_id,
              mapped.owned_category_id,
